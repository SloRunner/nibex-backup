...
pkgdatadir = $(datadir)/nibex
pkgincludedir = $(includedir)/nibex
pkglibdir = $(libdir)/nibex
pkglibexecdir = $(libexecdir)/nibex
...
CONFIG_HEADER = $(top_builddir)/src/config/nibex-config.h
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/nibex-config.h.in \
...
ACLOCAL = ${SHELL} /root/Nibex/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /root/Nibex/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /root/Nibex/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /root/Nibex/build-aux/missing automake-1.15
AWK = gawk
BDB_CPPFLAGS = 
...
LUPDATE = 
MAINT = 
MAKEINFO = ${SHELL} /root/Nibex/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL64 = 
PACKAGE = nibex
PACKAGE_BUGREPORT = www.nibex.co
PACKAGE_NAME = Nibex Core
PACKAGE_STRING = Nibex Core 1.0.0
PACKAGE_TARNAME = nibex
PACKAGE_URL = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /root/Nibex
abs_srcdir = /root/Nibex
abs_top_builddir = /root/Nibex
abs_top_srcdir = /root/Nibex
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /root/Nibex/build-aux/install-sh
...
SUBDIRS = src
GZIP_ENV = "-9n"
BITCOIND_BIN = $(top_builddir)/src/nibexd$(EXEEXT)
BITCOIN_QT_BIN = $(top_builddir)/src/qt/nibex-qt$(EXEEXT)
BITCOIN_CLI_BIN = $(top_builddir)/src/nibex-cli$(EXEEXT)
BITCOIN_WIN_INSTALLER = $(PACKAGE)-$(PACKAGE_VERSION)-win$(WINDOWS_BITS)-setup$(EXEEXT)
empty := 
space := $(empty) $(empty)
OSX_APP = Nibex-Qt.app
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_nibex_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info \
  leveldb_baseline_filtered.info test_nibex_coverage.info test_nibex.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/Nibex-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/nibex-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/nibex-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/nibex-config.h
$(top_srcdir)/src/config/nibex-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/nibex-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/Nibex-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Nibex-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/Nibex-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_nibex.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_nibex -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_nibex_filtered.info: test_nibex.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_nibex_filtered.info
#	$(MKDIR_P) qa/tmp
...
#test_nibex_coverage.info: baseline_filtered_combined.info test_nibex_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_nibex_filtered.info -o $@

#total_coverage.info:  baseline_filtered_combined.info test_nibex_filtered.info block_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_nibex_filtered.info -a block_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_nibex.coverage/.dirstamp:  test_nibex_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_nibex.coverage/.dirstamp total.coverage/.dirstamp

# NIBEX: disable the outdated comparison tool test
...
clean-local:
	rm -rf test_nibex.coverage/ total.coverage/ $(OSX_APP)
